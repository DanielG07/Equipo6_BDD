...
alter procedure numero_ventas_territorio_listar @territorio nvarchar (100) as
begin
...
begin
	...
	declare @nombreTienda nvarchar(100);
	declare @servidor nvarchar(100);
	declare @sql nvarchar(1000);
	declare @region nvarchar(100);
	...
	select @sql = N'select @tiendaOUT = [Name]
				from ['+@region+'].Sales.Store
				where BusinessEntityID = '+@tienda 

	set @Parametros = N'@tiendaOUT nvarchar (100) OUTPUT'

	exec sp_executesql @sql, @Parametros, @tiendaOUT=@nombreTienda OUTPUT
	...
	begin
		...
		begin
			 ...
			 set @sql =  'select '''+@nombreTienda+''' as ''Tienda'', st.Name as ''Territorio'', count(*) as ''Ventas'' 
...
begin
	...
	select @sql = N'select @ofertaOUT = SpecialOfferID
				from ['+@servidor+'].['+@region+'].Sales.SpecialOfferProduct
				where SpecialOfferID =' +@oferta+' and ProductID = '+@producto

	set @Parametros = N'@ofertaOUT nvarchar (5) OUTPUT'

	exec sp_executesql @sql, @Parametros, @ofertaOUT=@ofertaB OUTPUT

	if (isnull(@ofertaB,0) = 0)
	begin
		set @sql = 'insert into ['+@servidor+'].['+@region+'].Sales.SpecialOfferProduct(SpecialOfferID, ProductID, rowguid, ModifiedDate)
				values('+@oferta+', '+@producto+',newid(), getdate())' 
...
exec actualizar_oferta_producto '680', '8'
