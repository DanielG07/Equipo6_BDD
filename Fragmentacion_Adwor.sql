...
AS
if not exists(SELECT * FROM inserted i
              JOIN LSERVER2.AdventureWorks2019.Sales.Customer c
			  ON i.CustomerID = c.CustomerID)
ROLLBACK
...
AS
if not exists(SELECT * FROM inserted i
              JOIN LSERVER2.AdventureWorks2019.Sales.SalesPerson s
			  ON i.SalesOrderID = s.BusinessEntityID)
ROLLBACK
...
AS
if not exists(SELECT * FROM inserted i
              JOIN LSERVER2.AdventureWorks2019.Sales.SalesTerritory t
			  ON i.TerritoryID = t.TerritoryID)
ROLLBACK
...
AS
if not exists(SELECT * FROM inserted i
              JOIN LSERVER2.AdventureWorks2019.Purchasing.ShipMethod ps
			  ON i.ShipMethodID = ps.ShipMethodID)
ROLLBACK
...
AS
if not exists(SELECT * FROM inserted i
              JOIN LSERVER2.AdventureWorks2019.Sales.CreditCard cd
			  ON i.CreditCardID = cd.CreditCardID)
ROLLBACK
...
AS
if not exists(SELECT * FROM inserted i
              JOIN LSERVER2.AdventureWorks2019.Sales.CurrencyRate cr
			  ON i.CurrencyRateID = cr.CurrencyRateID)
ROLLBACK
...
AS
if not exists(SELECT * FROM inserted i
              JOIN LSERVER2.AdventureWorks2019.Production.Product p
			  ON i.ProductID = p.ProductID)
ROLLBACK
...
AS
if not exists(SELECT * FROM inserted i
              JOIN LSERVER2.AdventureWorks2019.Sales.SpecialOfferProduct so
			  ON i.SpecialOfferID = so.SpecialOfferID)
ROLLBACK
...
AS
if not exists(SELECT * FROM inserted i
              JOIN LSERVER2.AdventureWorks2019.Sales.SalesReason sri
			  ON i.SalesReasonID = sri.SalesReasonID)
ROLLBACK
